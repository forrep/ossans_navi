...
MAX_THREAD_TOKENS = 16000 if AI_SERVICE_TYPE == AiServiceType.GEMINI else 8000
...
REFINE_SLACK_SEARCHES_TOKEN = 30000 if AI_SERVICE_TYPE == AiServiceType.GEMINI else 24000

# lastshot で許容するトークン数
LASTSHOT_TOKEN_WITH_MENTION = 40000 if AI_SERVICE_TYPE == AiServiceType.GEMINI else 20000
# lastshot で許容するトークン数（メンションなし）
LASTSHOT_TOKEN_NO_MENTION = 20000 if AI_SERVICE_TYPE == AiServiceType.GEMINI else 10000
# lastshot で再入力する画像数
LASTSHOT_INPUT_IMAGE_FILES = (2 if AI_SERVICE_TYPE == AiServiceType.GEMINI else 0)
